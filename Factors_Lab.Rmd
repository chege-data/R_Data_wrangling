...
```{r 1.1response}
yts %>% ggplot(aes(x = Education, y = Sample_Size)) + geom_boxplot()
```
...
```{r 1.3response}
yts_factor <- yts %>% mutate(Education = factor(Education, levels = c("Middle School","High School")))
yts_factor
```
...
```{r 1.4response}
yts_factor %>% ggplot(aes(x = Education, y = Sample_Size)) + geom_boxplot()

yts_factor %>% count(Education)
```
